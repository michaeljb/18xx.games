    ...
    module G1840
      module Step
        class AcquisitionAuction < Engine::Step::Base
          ...
          def log_pass(entity)
            message = "#{log_with_player(entity)} passes bidding"
            message << "on #{@auctioning.name}" if @auctioning
            @log << message
          end

          def log_skip(entity)
            @log << "#{log_with_player(entity)} cannot afford bidding on #{@auctioning.name}"
          end
          ...
          def process_pass(action)
            ...
            if auctioning
              ...
              @log << "#{log_with_player(entity)} passes bidding"
              entity.pass!
          ...
          def process_merge(action)
            ...
            @log << "#{log_with_player(action.entity)} selects #{action.corporation.full_name} for auctioning"
          end
          ...
          def add_bid(bid)
            ...
            @log << "#{log_with_player(entity)} bids #{@game.format_currency(price)} for #{corporation.name}"
          ...
          def win_bid(winner, _company)
            ...
            @log << "#{log_with_player(buying_corporation)} wins the auction for #{corporation.name} "\
