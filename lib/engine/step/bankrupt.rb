    ...
    class Bankrupt < Base
      ...
      def process_bankrupt(action)
        ...
        unless @game.can_go_bankrupt?(player, corp)
          buying_power = @game.format_currency(@game.total_buying_power(player, corp))
          price = @game.format_currency(@game.depot.min_depot_price)

          msg = "Cannot go bankrupt. #{corp.name} and #{player.name} have a total buying power of "\
                "#{buying_power}, and the cheapest train in the Depot costs #{price}."
          @game.game_error(msg)
        end
