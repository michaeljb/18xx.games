...
module Engine
  module Game
    ...
    class Base
      ...
      # Once a game end reason has been met, stop checking the other possible
      # game end reasons
      ...
      PLAYER_LOAN_PENALTY_INTEREST = 0
          ...
          when 'undo'
            undo_to = if (id = action['action_id'])
                        if id.zero?
                          ...
                          raise GameError, "Could not find target action_id for undo action: #{action}" if idx.nil?
      ...
      def take_player_loan(
        ...
        penalty = (amount * self.class::PLAYER_LOAN_PENALTY_INTEREST / 100.0).ceil
