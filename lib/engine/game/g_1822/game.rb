  ...
  module Game
    module G1822
      class Game < Game::Base
        ...
        def minors_to_float
          bidbox_minors.filter_map.with_index do |minor, bb_index|
            next unless (minor_bid = @round.highest_bid(minor))

            [minor_bid, bb_index]
          end.sort_by { |mb, bb_index| [mb.price, -bb_index] }.reverse
        end

        def minor_float_index(company)
          minors_to_float.find_index { |mb, bb_idx| mb.company == company }
        end
        ...
        def minor_float_status_train_export(company)
          ...
          bidless_minors = bidbox_minors.select { |m| !@round.highest_bid(m) }
