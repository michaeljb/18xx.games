  ...
  module Game
    module G1868WY
      class Game < Game::Base
        ...
        MOUNTAIN_BORDER_HEXES = %w[L2 M3].freeze
        CASPER_HEX = 'H18'
        ...
        def setup
          ...
          city_by_id('N18-0-0').place_token(neutral, neutral.next_token)
        ...
        def event_close_pure_oil_camp!
          @log << "Company #{pure_oil.name} closes"
          pure_oil.close!
          @pure_oil_camp_hex&.remove_assignment!(pure_oil.id)
          to_ghost_town!(@pure_oil_camp_hex)
        end
        ...
        def spend_tile_lay_points(action)
          return unless action.entity.corporation? || action.entity == lander
        ...
        def preprocess_action(action)
          case action
          when Action::LayTile
            @border_before = action.hex.tile.borders.first if MOUNTAIN_BORDER_HEXES.include?(action.hex.name)
          end
        end
        ...
        def action_processed(action)
          ...
          when Action::LayTile
            if action.hex.name == WIND_RIVER_CANYON_HEX
              swap_color_and_stripe(action.hex.tile)
            else
        ...
        def corp_stacks_str_arr(arr)
          @corp_stacks.map.with_index do |stack, index|
        ...
        def sr_visible_corporations
          ...
          [*corporations.select(&:ipoed).sort, *@corp_stacks.flat_map(&:last)]
        end
        ...
        def place_development_token(action)
          ...
          if entity == union_pacific_coal || entity == bonanza
            ...
            if (placed_up_token = entity.tokens.find(&:used))
              from_hex = placed_up_token.hex.name
              destroy_development_token!(placed_up_token)
        ...
        def upgrades_to?(from, to, special = false, selected_company: nil)
          ...
          elsif %w[8 9].include?(from.name) && %w[16 19 20].include?(to.name)
            {
              '8' => %w[16 19],
              '9' => %w[19 20],
            }[from.name].include?(to.name)
          else
            super
          end
        end

        def new_tile_cost(tile)
          (%w[16 19 20].include?(tile.name) ? 20 : 0)
        end
        ...
        def revenue_for(route, stops)
          includes_yellowstone = false

          revenue = stops.sum do |stop|
            includes_yellowstone = true if !includes_yellowstone && YELLOWSTONE_HEXES.include?(stop.hex.id)
          ...
          revenue += fort_bonuses(route, stops)[:revenue]
        ...
        def revenue_str(route)
          ...
          str += fort_bonuses(route, route.stops)[:description]
        ...
        def boomcity_to_boomtown!(hex)
          ...
          token = Token.new(
            union_pacific,
            price: 0,
            logo: union_pacific.logo,
            simple_logo: union_pacific.simple_logo,
            type: :boomcity_reservation,
          )
          hex.place_token(token, logo: token.simple_logo)
        end
        ...
        def next_round!
          @round =
            ...
            when G1868WY::Round::Operating
              ...
              last_or_in_set || @endgame_triggered ? new_bust_round : new_development_round(@round.round_num + 1)
            when G1868WY::Round::Bust
              resolve_busters!
              if @endgame_triggered && @final_stock_round_started
                new_development_round(@round.round_num + 1)
              else
        ...
        def place_pure_oil_camp(hex)
          pure_oil_camp = pure_oil
          type = @development_token_count[hex] < DTC_BOOMCITY ? :boomtown : :boomcity

          @pure_oil_camp_hex = hex
          ...
          if hex.tile.color == :white
            ...
            when :boomcity
              tile.add_reservation!(pure_oil_camp.owner, 0, 0)
              @log << "#{hex.name} reserved for #{pure_oil_camp.owner.name}"
            end
          end
        end
        ...
        def check_midwest_oil!(routes)
          ...
          return unless midwest_oil.owned_by_player? || midwest_oil.owned_by_corporation?
        ...
        def foncier_check_tile_lay_action(action)
          return if !foncier || foncier.closed? || foncier.player.nil?
          ...
          amount, tile_type =
            if boomer?(tile)
              ...
              [40, 'city']
            elsif tile.color == :yellow && !tile.towns.empty?
              [10, 'yellow town']
            else
              [0, nil]
            end
        ...
        def total_rounds(name)
          case name
          when self.class::PRIVATES_ROUND_NAME
            2
          when self.class::DEVELOPMENT_ROUND_NAME, self.class::OPERATING_ROUND_NAME
            @operating_rounds
          end
        end
        ...
        def fort_bonuses(route, _stops)
          return { revenue: 0, description: '' } unless route.corporation == trabing_bros.owner

          forts = route.connection_hexes.flatten.uniq.count do |hex_id|
            hex_by_id(hex_id).tile.icons.find { |icon| icon.name == 'fort' }
          end

          if forts.positive?
            { revenue: 10 * forts, description: ' + Forts' }
          else
            { revenue: 0, description: '' }
          end
        end
