          ...
          r.is 'action' do
            ...
            DB.with_advisory_lock(:action_lock, game.id) do
              if game.settings['pin']
                ...
                engine = Engine::Game.load(game, actions: actions_h(game), strict: false)
                prev = acting_users(engine, users)

                r.params['user'] = user.id

                strict = engine.use_engine_v2
                engine = engine.process_action(r.params, validate_auto_actions: true, strict: strict)
